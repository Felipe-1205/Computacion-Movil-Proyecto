...
import com.example.proyecto.databinding.ActivityMainEncargadoBinding
...
class Main_encargado : AppCompatActivity() {

    private lateinit var binding: ActivityMainEncargadoBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMainEncargadoBinding.inflate(layoutInflater)
        setContentView(binding.root)

        mAuth = FirebaseAuth.getInstance()
        database = FirebaseDatabase.getInstance()
        myRef = database.getReference("users")
        binding.lottieAnimationView2.resumeAnimation()
        setupLocation()
        when {
        ...
        myRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                users.clear()
                for (correo in usuarioactual.usuarios){
